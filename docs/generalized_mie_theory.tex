...
For example, if the incident field is right-hand-circularly polarized and a mirror symmetry is imposed, the mirrored fields will be left-hand-circularly polarized.
This can be corrected by including a $\pi$ phase factor in the mirror symmetry.
...
\begin{equation}
    p^{\prime j,\text{inc}}_{mnr} = \Phi (-1)^{m+1} p^{j,\text{inc}}_{mnr}
...
\begin{enumerate*}
    \item \textbf{version 0.3} (released)
        \begin{itemize}[label={\tiny\raisebox{1ex}{\textbullet}}]
            ...
            \item Plane-wave/beams arbitrary direction and polarization control
        \end{itemize}
