...
\begin{align}
   ...
   -\boldsymbol{\hat \phi} \tau_{mn}(\cos\theta)z_n^{(J)}(kr) e^{im\phi} \\
    \boldsymbol{N}_{mn}^{(J)} &= 
...
\end{split}
\end{align}
where $\xi_n^{(1)} = \psi_n - i \chi_n$, and $\psi_n$, $\chi_n$ are the Riccati-Bessel function of the first and second kind
\begin{align}
\begin{split}
    \psi_n(x) &= xj_n(x) \\
    \chi_n(x) &= -xy_n(x) \\
...
Denoting the coefficients of our convention $\bar p_{mn}$, $\bar q_{mn}$, $\bar a_{mn}$, $\bar b_{mn}$, the two conventions are related by
\begin{align}
\begin{split}
    \bar p_{mn} &= ? p_{mn} \\
    \bar q_{mn} &= ? q_{mn} \\
