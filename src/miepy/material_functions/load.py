...
def plot_material_by_author(material_name, wavelength_min=0, wavelength_max=np.inf):
    ...
    for author in authors:
        ...
        ax1.plot(data["wavelength"] * 1e9, data["eps"].real, label=author, color=color, linewidth=1, marker=marker)
        ax2.plot(data["wavelength"] * 1e9, data["eps"].imag, label=author, color=color, linewidth=1, marker=marker)
