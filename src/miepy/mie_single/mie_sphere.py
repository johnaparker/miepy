...
class single_mie_sphere:
    def __init__(self, radius, material, wavelength, lmax, medium=None):
        ...
        if medium is None:
            ...
            self.medium = medium
            if (self.medium.eps(self.wavelength).imag != 0).any() or (self.medium.mu(self.wavelength).imag != 0).any():
    ...
    def cross_sections_per_multipole(self):
        ...
        return miepy.flux.cross_sections(
            miepy.scattering_per_multipole(*self.scattering_properties),
    ...
    def radiation_force(self):
        ...
        for i in range(self.lmax):
            ...
            if n < self.lmax:
                ...
                Fz -= factor * np.real(
                    self.an[:, i] * np.conj(self.an[:, i + 1]) + self.bn[:, i] * np.conj(self.bn[:, i + 1])
                )
