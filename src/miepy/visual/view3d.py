...
def get_paint(material):
    ...
    if material.name == "Ag":
        return paint(vpython.textures.metal, vpython.vec(*[0.9] * 3), shininess=0.15)
    if material.name == "Au":
...
def draw_particle(particle, paint, **kwargs):
    ...
    if type(particle) is miepy.sphere:
        ...
        axis = vec(np.sin(theta) * np.cos(phi), np.sin(theta) * np.sin(phi), np.cos(theta))
        ...
        if type(particle) is miepy.cylinder:
            return vpython.cylinder(
                pos=(vec(*particle.position) - particle.height / 2 * axis) / nm,
                radius=particle.radius / nm,
                length=particle.height / nm,
                texture=paint.texture,
                axis=axis,
                color=paint.color,
                shininess=paint.shininess,
                **kwargs,
            )

        elif type(particle) is miepy.spheroid:
            return vpython.ellipsoid(
                pos=vec(*particle.position) / nm,
