    ...
    def get_Q(p, q):
        ...
        for r1, _n1, _m1 in miepy.mode_indices(lmax):
            ...
            for r2, _n2, _m2 in miepy.mode_indices(lmax):
                ...
                integrand = np.sum(np.cross(dS, M2_q_k2, axis=0) * N1_p_k1, axis=0) + np.sqrt(eps / eps_m) * np.sum(
                    np.cross(dS, N2_q_k2, axis=0) * M1_p_k1, axis=0
                )
                integrand *= 1j * k1**2 / np.pi * factor
                Q[1, r1, 1, r2] = miepy.vsh.misc.trapz_2d(theta, phi, integrand)

                integrand = np.sum(np.cross(dS, N2_q_k2, axis=0) * N1_p_k1, axis=0) + np.sqrt(eps / eps_m) * np.sum(
                    np.cross(dS, M2_q_k2, axis=0) * M1_p_k1, axis=0
                )
                integrand *= 1j * k1**2 / np.pi * factor
                Q[1, r1, 0, r2] = miepy.vsh.misc.trapz_2d(theta, phi, integrand)

                integrand = np.sum(np.cross(dS, M2_q_k2, axis=0) * M1_p_k1, axis=0) + np.sqrt(eps / eps_m) * np.sum(
                    np.cross(dS, N2_q_k2, axis=0) * N1_p_k1, axis=0
                )
                integrand *= 1j * k1**2 / np.pi * factor
                Q[0, r1, 1, r2] = miepy.vsh.misc.trapz_2d(theta, phi, integrand)

                integrand = np.sum(np.cross(dS, N2_q_k2, axis=0) * M1_p_k1, axis=0) + np.sqrt(eps / eps_m) * np.sum(
                    np.cross(dS, M2_q_k2, axis=0) * N1_p_k1, axis=0
                )
                integrand *= 1j * k1**2 / np.pi * factor
...
def ellipsoid_dS(a, b, c, theta, phi):
    rad = 1 / np.sqrt(
        np.sin(theta) ** 2 * np.cos(phi) ** 2 / a**2
        + np.sin(theta) ** 2 * np.sin(phi) ** 2 / b**2
        + np.cos(theta) ** 2 / c**2
    )
    dr_theta = -(rad**3) * (
        np.sin(theta) * np.cos(theta) * (np.cos(phi) ** 2 / a**2 + np.sin(phi) ** 2 / b**2 - 1 / c**2)
    )
