...
class gaussian_beam(polarized_beam):
    def __init__(
        self, width, polarization, power=1, theta_max=np.pi / 2, phase=0, center=None, theta=0, phi=0, standing=False
    ):
        super().__init__(
    ...
    def __repr__(self):
        return (
            f"gaussian_beam(width={self.width}, polarization={self.polarization}, power={self.power}, "
...
class bigaussian_beam(polarized_beam):
    ...
    def __repr__(self):
        return (
            f"bigaussian_beam(width_x={self.width_x}, width_y={self.width_y}, polarization={self.polarization}, "
            f"power={self.power}, center={self.center}, theta={self.theta}, phi={self.phi})"
        )

    def scalar_angular_spectrum(self, theta, phi, k):
        return np.exp(
            -((k * np.tan(theta) / 2) ** 2) * ((self.width_x * np.cos(phi)) ** 2 + (self.width_y * np.sin(phi)) ** 2)
        )
...
class hermite_gaussian_beam(polarized_beam):
    ...
    def __repr__(self):
        return (
            f"HG_beam(width={self.width}, l={self.l}, m={self.m}, polarization={self.polarization}, "
            f"power={self.power}, center={self.center}, theta={self.theta}, phi={self.phi})"
        )
...
class laguerre_gaussian_beam(polarized_beam):
    ...
    def __repr__(self):
        return (
            f"LG_beam(width={self.width}, p={self.p}, l={self.l}, polarization={self.polarization}, "
            f"power={self.power}, center={self.center}, theta={self.theta}, phi={self.phi})"
        )
...
def azimuthal_beam(width, theta=0, phi=0, power=None, phase=0, center=None, theta_max=np.pi / 2):
    ...
    HG_1 = hermite_gaussian_beam(
        1, 0, width, [0, -1], theta=theta, phi=phi, power=power / 2, phase=phase, center=center, theta_max=theta_max
    )
    HG_2 = hermite_gaussian_beam(
        0, 1, width, [1, 0], theta=theta, phi=phi, power=power / 2, phase=phase, center=center, theta_max=theta_max
    )
    return HG_1 + HG_2
...
def radial_beam(width, theta=0, phi=0, power=None, phase=0, center=None, theta_max=np.pi / 2):
    ...
    HG_1 = hermite_gaussian_beam(
        1, 0, width, [-1, 0], theta=theta, phi=phi, power=power / 2, phase=phase, center=center, theta_max=theta_max
    )
    HG_2 = hermite_gaussian_beam(
        0, 1, width, [0, -1], theta=theta, phi=phi, power=power / 2, phase=phase, center=center, theta_max=theta_max
    )
    return HG_1 + HG_2
