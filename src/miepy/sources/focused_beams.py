...
def I_generic(func, k, theta_max, f, w0):
    ...
    def compute_integral(rho, z):
        def integral(theta):
            integrand = (
                filling_factor(f, w0, theta, theta_max)
                * np.cos(theta) ** 0.5
                * func(theta, rho)
                * np.e ** (1j * k * z * np.cos(theta))
            )
            return integrand

        return (
            integrate.quad(lambda theta: integral(theta).real, 0, theta_max)[0]
...
def I00(k, theta_max, f, w0):
    return I_generic(
        lambda theta, rho: np.sin(theta) * (1 + np.cos(theta)) * j0(k * rho * np.sin(theta)), k, theta_max, f, w0
    )


def I01(k, theta_max, f, w0):
    return I_generic(lambda theta, rho: np.sin(theta) ** 2 * j1(k * rho * np.sin(theta)), k, theta_max, f, w0)


def I02(k, theta_max, f, w0):
    return I_generic(
        lambda theta, rho: np.sin(theta) * (1 - np.cos(theta)) * jv(2, k * rho * np.sin(theta)), k, theta_max, f, w0
    )


def I10(k, theta_max, f, w0):
    return I_generic(lambda theta, rho: np.sin(theta) ** 3 * j0(k * rho * np.sin(theta)), k, theta_max, f, w0)
...
def I12(k, theta_max, f, w0):
    return I_generic(
        lambda theta, rho: np.sin(theta) ** 2 * (1 - np.cos(theta)) * j1(k * rho * np.sin(theta)), k, theta_max, f, w0
    )


def I13(k, theta_max, f, w0):
    return I_generic(lambda theta, rho: np.sin(theta) ** 3 * jv(2, k * rho * np.sin(theta)), k, theta_max, f, w0)
...
def Irad(k, theta_max, f, w0):
    return I_generic(
        lambda theta, rho: np.cos(theta) * np.sin(theta) ** 2 * j1(k * rho * np.sin(theta)), k, theta_max, f, w0
    )
