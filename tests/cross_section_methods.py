...
for i, wavelength in enumerate(tqdm(wavelengths)):
    ...
    for j in range(dimer.Nparticles):
        ...
        Ap[i] += np.sum(
            miepy.flux.cluster_cross_sections(dimer.p_scat[j], dimer.p_inc[j], dimer.material_data.k_b).absorption
        )

        ### extinction is calculated using p_src
        Ep[i] += np.sum(
            miepy.flux.cluster_cross_sections(dimer.p_scat[j], dimer.p_src[j], dimer.material_data.k_b).extinction
        )
