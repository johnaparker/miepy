...
def pi(theta, Nmax):
    ...
    for n in range(3,Nmax+1):
        ...
        for m in range(0, n-1):
            mid = n + m
            values[nid,mid] = (2*n - 1)/(n - m)*x*values[nid-1,mid-1] \
                              - (n + m - 1)/(n - m)*values[nid-2,mid-2]
        ...
        mid = n + m
        if m != 2:
            values[nid,mid] = -m*(n+m-1)*(n-m+2)/(m-2)*values[nid,mid-1] \
                            + 2*m*x/(1-x**2)**0.5*values[nid,mid-2]
        else:
            values[nid,mid] = (values[nid,mid+1] + (m+1)*(n+m)*(n-m+1)/(m-1)*values[nid,mid-1]) \
                            * (1-x**2)**0.5/(2*(m+1)*x)
