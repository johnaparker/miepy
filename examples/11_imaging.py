...
ax = axes[2]
x = np.linspace(-xmax, xmax, 80)
y = np.linspace(-xmax, xmax, 80)
X, Y = np.meshgrid(x, y)

scope = miepy.microscope(cluster, focal, theta_obj, sampling=60)
im = scope.image(x, y, 0)
