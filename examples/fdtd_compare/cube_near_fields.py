...
vertices = [meep.Vector3(X, 0, 0),
            meep.Vector3(-X/2, np.sqrt(3)/2*X, 0),
            meep.Vector3(-X/2, -np.sqrt(3)/2*X, 0)]

vertices = [meep.Vector3(W/2, -W/2, 0),
            meep.Vector3(W/2, +W/2, 0),
            meep.Vector3(-W/2, +W/2, 0),
            meep.Vector3(-W/2, -W/2, 0)]

geometry.append(meep.Prism(center=meep.Vector3(0, 0, 0),
                          height=H,
                          vertices=vertices,
...
def sim():
    ...
    sim.run(until_after_sources=meep.stop_when_fields_decayed(.5*um, decay,
                pt=meep.Vector3(0,0,box[2]/2), decay_by=1e-5))
    # for i in range(100):
        # sim.fields.step()
