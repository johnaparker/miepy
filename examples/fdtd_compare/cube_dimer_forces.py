...
def force_gmmt():
    ...
    particles = [miepy.cube([-sep/2,0,0], 200*nm, material),
...
def vis():
    ...
    for j,c in enumerate(['x', 'y', 'z']):
        ax = axes[j]
        key = f'F{c}'
        ax.plot((1/nm)/norm.frequency, scat[key]/norm.incident*norm.area*constants.epsilon_0/2*1e25, 'o', color=f'C{j}', label=f'F{c} (FDTD)')
